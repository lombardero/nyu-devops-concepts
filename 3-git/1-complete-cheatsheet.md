...
- Relevant commands (click to go to each): [`git init`](#10-initializing), [`git status`](#checking-status), [`git diff`](#checking-status), [`git add`](#adding-files-to-staging-area), [`git commit`](#12-comitting-files-from-staging-area-to-local-repository), [`git log`](#retrieving-commit-history)

### [2: Working with remote repositories](#2-working-with-remote-repositories-1)
- Relevant commands (click to go to each): [`git clone`](#21-cloning-remote-repositories), [`git remote add`](#22-addingchecking-remote-repositories), [`git fetch`](#23-pulling-data-from-remote-repositories), [`git merge`](#23-pulling-data-from-remote-repositories), [`git push`](#24-pushing-data-upstream)

### [3: Working with branches](#3-working-with-branches-1)
- Relevant commands (click to go to each): [`git checkout`](#31-creating-and-switching-branches), [`git merge`](#32-merging-branches), [`git branch`](#31-creating-and-switching-branches), [`git rebase`](#33-rebasing)
...
- takes the data from a branch in the local `.git/` folder and merges it into our local files in the workspace (merging might bring discrepancies - i.e. a line has been modified in the `.git/` folder and locally (in the workspace), they must be resolved before being able to end the merge command, see [how to solve merge conflicts](#sorting-out-merge-conflicts)).
...
### Successful merges
```git merge [branchname]```
- merges the specified branch with the branch you are currently in (takes the data from our local `./git` folder on that branch, and changes the file in the workspace; this might bring merge conflicts, see how to resolve them [here](#sorting-out-merge-conflicts))
> Note: `git merge [branchname]` merges `[branchname]` with the branch you are currently in. If you are in a branch called `branch1` and you run `git merge branch2`, you will merge the changes made in `branch2` in the `branch1`; in another words: `branch1` will be modified. 
