...
The hard drive is the 'long-term' storage of computers. The data is saved in a persistent way (it is not lost if the machine is turn off), but it is also slower to access than the RAM. Data such as Software packages, Photos, or Music is stored in the Hard Drive.
...
'Saving the state' of a machine simply means saving the contents of the RAM into the Hard Drive, so it can be recovered later. The data in the RAM, also called the 'state' since it is not recoverable if lost, can be stored securely so that 'intermediate steps' of computations can be stored. 

Docker containers, for example, always start from a 'saved state' of a previously generated container (which has the software we want already installed and running), in order to run the components we want as fast as possible
...
This part overviews how the software controls the hardware. The goal of this part is to understand very basically how a computer is controlled, and what is a Virtual Machine (VM). For that, we need to intuitively understand what is the kernel, and and an hypervisor.
...
#### Hypervisors
Each operating system has its own 'language', and way of storing devices. A **hypervisor** is simply a 'translator layer' that allows a Host Operating System be run in any computer, believing it runs in a physical device. To achieve this, hypervisors perform two main tasks:
- They 'freeze' computer resources (CPU, RAM and Hard Disk) so that the virtual machine can use them. It achieves this by partitioning them: when running a VM, the RAM, for example, will be split in two: one 'slice' for the Host OS, and another for the VM (A RAM of 8GB might be split in 4GB for the Host OS, and 4GB for the VM, for example).
- 'Translating' orders from one operating system to another. For example, if we emulate a UNIX operating system (as we will do on the course), it will use a different 'language' to talk to the hardware than the Windows or MAC Operating System in our machines. Hypervisors 'translate' UNIX commands (such as 'give me 10MB of RAM for this process') so that a Windows or a MAC operating system can understand it and perform it, in such a way so that the UNIX system 'believes' that is running in an actual computer (although it is not).
...
![alt text](https://github.com/lombardero/nyu-devops-concepts/tree/master/0-basic-concepts/assets/VM-diagram.png "Port Forwarding 1")
...
VMs are used in DevOps (as well as containers) to create a replicable evironment (such as a UNIX system), so that any member of the team can re-create it without problem and work on it, to avoid the "it only works on your computer" kinds of problem. In the Class, we will use `Vagrant` to create the specifications of the VM we want to start.

VMs are one of the keys for the revolution in Cloud Computing: all instances run the Cloud are VMs running somewhere in a Data Center. Thanks to them, computing has become much more efficient (and that has saved billions of dollars to many companies). Now, instead of having to buy a physical computer to run a server, multiple servers are 'squished' into a computer thanks to virtualization in a 'pay-for-what-you-need' business model.


## 2 - Networking Basics
In this course, we will be running a server, and use a nice trick called 'port-forwarding' to access the outputs of the server we will run in a VM. For that, we need to understand some very basics of Networking, such as what are IPs, ports, and very basics of how HTML (the way servers talk to browsers) works.
...
'Port forwarding' is a trick we will use to communicate with a server running in a Virtual Machine (as we will do in the Class project).
...
![alt text](https://github.com/lombardero/nyu-devops-concepts/tree/master/0-basic-concepts/assets/port-fwd1.png "Port Forwarding 1")
...
### 2.2 HTTP protocol
`HTTP` stands for 'Hypertext Transfer Protocol', which is used to send and receive data through the internet, and the language used by servers and browsers to communicate. HTTP codifies the messages sent by clients (ex: a browser) in a structured manner, so that servers can understand what the client is asking for, and send a response to it.
...
The **header** should store two or three key pieces of data (depending on the HTT):
- The HTTP method (explained above)
- The URL requested (explained above)
- The `Content-Type`: which is required only when the `POST` and `PUT`, and allows the server to know which type of data the client is sending. (example: setting `Content-Type: application/json` will let the server know we are sending a JSON object).

The **body** contains the data itself (example: a JSON object).
...
The **body** is the content of the message the server wants to send back to the client. It can have many formats: plain text, HTML, JSON... The browser understands those languages and will render the response properly.

#### 2.2.3 the `JSON` format
JSON stands for 'JavasCript Object Notation', which is a type of notation very commonly used to send data through HTTP (since browsers understand `JavaScript`). JSON objects are very similar to Python dictionnaries: the data is stored in a structured way by slicing the data in set of keys.

##### How does a JSON object look like?
Imagine we are a client that wants to purchase a music instrument. The client (browser) will send a request to the server to buy that product. For simplicity, let's say the server only requires two fields to identify the product and process its purchase: the `product_name` and its `id_number`). The **body** of the message the browser will send to the server is shown below:
```JSON
{
...
```
- The object above is in JSON format (as simple as that)
